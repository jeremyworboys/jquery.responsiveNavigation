...
(function($) {
    ...
    test("creates groups using default options", function() {
        ok($("#test-header").responsiveNavigation(), "runs with no options passed");
        equal($("#test-header").find(".responsive-navigation-wrapper select optgroup").length, 2, "groups are created");
    ...
    test("creates a group for one nav if specified", function() {
        ...
        equal($("#test-header").find(".responsive-navigation-wrapper select optgroup").length, 1, "one group is created");
        equal($("#test-header").find(".responsive-navigation-wrapper select option").length, 4, "select element contains all links + the default option");
    });
    ...
    test("groups nested list navs correctly", function() {
        ...
        equal($("#test-header-with-lists").find(".responsive-navigation-wrapper select optgroup").length, 3, "groups are created");
        equal($("#test-header-with-lists").find(".responsive-navigation-wrapper select option").length, 8, "select element contains all links + the default option");
